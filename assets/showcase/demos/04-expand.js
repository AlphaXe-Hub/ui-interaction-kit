...
(function () {
  ...
  UIK.register('expand', {
    ...
    mount: function (stage, ctx) {
      ...
      U.css(el, {
        ...
        background: 'var(--d-accent)', color: 'var(--d-inv-text)', fontWeight: '600',
  ...
  UIK.register('expand', {
    ...
    mount: function (stage, ctx) {
      ...
      var el = U.el('div'); U.css(el, {
        width: '220px', height: '36px', borderRadius: '18px', background: 'var(--d-panel)', border: '1px solid var(--d-border)',
        overflow: 'hidden', cursor: 'pointer', position: 'relative'
      });
      UIK.tx(el, 'width .42s cubic-bezier(.3,1.2,.5,1),height .42s cubic-bezier(.3,1.2,.5,1),border-radius .42s');
      var line = U.el('div'); U.css(line, { display: 'flex', alignItems: 'center', height: '36px', padding: '0 14px', fontSize: '12px', color: 'var(--d-dim)' });
      line.innerHTML = '<span style="width:8px;height:8px;border-radius:50%;background:var(--d-ok);margin-right:8px"></span>同步已完成 · 查看细节';
      ...
      body.innerHTML = '<div style="font-size:13px;font-weight:600;margin-bottom:4px">同步详情</div>' +
        '<div style="font-size:11.5px;color:var(--d-dim)">共 128 条记录，失败 2 条。展开后显示完整信息与操作按钮。</div>' +
  ...
  UIK.register('expand', {
    ...
    mount: function (stage, ctx) {
      var host = stageBox(stage);
      var el = U.el('div'); U.css(el, {
        background: 'var(--d-panel)', border: '1px solid var(--d-border)', borderRadius: '22px', overflow: 'hidden', width: '196px'
      });
      UIK.tx(el, 'height .4s cubic-bezier(.3,1.1,.5,1),border-radius .4s');
      el.style.height = '40px';
      var bar = U.el('div'); U.css(bar, { display: 'flex', alignItems: 'center', gap: '10px', height: '40px', padding: '0 14px', fontSize: '12px', color: 'var(--d-dim)' });
      ...
      p.innerHTML = '<input type="range" style="width:100%;accent-color:var(--d-accent)">' +
        ...
        '<label style="display:flex;gap:8px;align-items:center;margin-top:10px;font-size:11.5px;color:var(--d-dim)"><input type="checkbox" checked> 循环播放</label>';
  ...
  UIK.register('expand', {
    ...
    mount: function (stage, ctx) {
      var host = stageBox(stage);
      var el = U.el('div'); U.css(el, {
        width: '220px', height: '130px', background: 'var(--d-panel)', border: '1px solid var(--d-border)',
      ...
      el.innerHTML = '<div style="font-size:13px;font-weight:600">形态过渡</div><div style="font-size:11.5px;color:var(--d-dim)">尺寸不变，只改圆角与阴影</div>';
      host.appendChild(el);
      toggleBtn(host, '变为弹窗形态', '变回卡片形态', function (on) {
        ...
        el.style.background = on ? 'var(--d-panel)' : 'var(--d-panel)';
      });
  ...
  UIK.register('expand', {
    ...
    mount: function (stage, ctx) {
      ...
      var el = U.el('div'); U.css(el, {
        ...
        background: 'var(--d-panel)', border: '1px solid var(--d-border)', padding: '12px'
      });
      ...
      var more = U.el('div'); U.css(more, { marginTop: '8px', opacity: '0', transition: UIK.isReduced() ? 'none' : 'opacity .3s .15s', fontSize: '11.5px', color: 'var(--d-dim)' });
  ...
  UIK.register('expand', {
    ...
    mount: function (stage, ctx) {
      var host = stageBox(stage);
      var el = U.el('div'); U.css(el, {
        width: '260px', background: 'var(--d-panel)', border: '1px solid var(--d-border)', borderRadius: '12px', padding: '10px',
      ...
      var chips = ['需求', '设计', '开发', '测试', '上线', '验收'].map(function (t, i) {
        var c = U.el('span'); U.css(c, {
          fontSize: '11px', color: 'var(--d-dim)', background: 'var(--d-chip)', border: '1px solid var(--d-border)',
  ...
  UIK.register('expand', {
    ...
    mount: function (stage, ctx) {
      ...
      var panel = U.el('div'); U.css(panel, {
        ...
        background: 'var(--d-panel)', border: '1px solid var(--d-border)', borderRadius: '12px', opacity: '0',
      ...
      panel.innerHTML = '<div style="padding:12px;font-size:11.5px;color:var(--d-dim)">沿原路径收回：高度、缩放与透明度同时回到起点。</div>';
