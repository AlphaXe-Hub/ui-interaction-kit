...
(function () {
  ...
  UIK.register('nav', {
    ...
    mount: function (stage, ctx) {
      ...
      var bar = U.el('div'); U.css(bar, { display: 'flex', gap: '4px', borderBottom: '1px solid #2b3348' });
      bar.setAttribute('role', 'tablist');
      var tabs = ['基本信息', '跟进记录 (12)', '附件 (3)'].map(function (t, i) {
        var b = U.el('button'); b.type = 'button';
        U.css(b, {
          background: 'transparent', border: 'none', color: '#98a1b8', padding: '8px 12px', fontSize: '12.5px',
          cursor: 'pointer', borderBottom: '2px solid transparent', marginBottom: '-1px'
        });
        b.textContent = t; b.setAttribute('role', 'tab');
        bar.appendChild(b); return b;
      });
      var panel = U.el('div'); U.css(panel, { padding: '14px 4px', fontSize: '12px', color: '#98a1b8', minHeight: '74px' });
      ...
      function go(i) {
        ...
        tabs.forEach(function (b, k) {
          b.style.color = k === i ? '#e6e9f2' : '#98a1b8';
          b.style.borderBottomColor = k === i ? '#6ea8fe' : 'transparent';
  ...
  UIK.register('nav', {
    ...
    mount: function (stage, ctx) {
      ...
      var seg = U.el('div'); U.css(seg, { position: 'relative', display: 'flex', background: '#121724', border: '1px solid #2b3348', borderRadius: '10px', padding: '3px' });
      var slider = U.el('div'); U.css(slider, {
        position: 'absolute', top: '3px', bottom: '3px', left: '3px', width: '0',
        background: '#2a3348', borderRadius: '8px', transition: UIK.isReduced() ? 'none' : 'left .3s cubic-bezier(.4,0,.2,1),width .3s cubic-bezier(.4,0,.2,1)'
      });
      seg.appendChild(slider);
      var btns = ['日', '周', '月'].map(function (t, i) {
        var b = U.el('button'); b.type = 'button';
        U.css(b, { position: 'relative', zIndex: 1, background: 'transparent', border: 'none', color: '#98a1b8', padding: '7px 18px', fontSize: '12.5px', cursor: 'pointer', minHeight: '34px' });
        b.textContent = t; seg.appendChild(b); return b;
      });
      var view = U.el('div'); U.css(view, { fontSize: '12px', color: '#98a1b8' });
      ...
      btns.forEach(function (b, i) {
        ctx.on(b, 'click', function () {
          btns.forEach(function (o, k) { o.style.color = k === i ? '#e6e9f2' : '#98a1b8'; });
    ...
    mount: function (stage, ctx) {
      ...
      function render() {
        ...
        path.forEach(function (p, i) {
          if (i > 0) { var s = U.el('span'); s.textContent = '/'; U.css(s, { color: '#3a4255' }); nav.appendChild(s); }
          if (i === path.length - 1) {
            var cur = U.el('span'); cur.textContent = p; U.css(cur, { color: '#e6e9f2' });
            cur.setAttribute('aria-current', 'page'); nav.appendChild(cur);
          } else {
            var a = U.el('a', null, p); U.css(a, { color: '#6ea8fe', cursor: 'pointer', textDecoration: 'none' });
      ...
      function render() {
        ...
        function btn(label, page, disabled, active) {
          ...
          U.css(b, {
            ...
            background: active ? '#6ea8fe' : '#161a24', color: active ? '#0b0d12' : (disabled ? '#4b5470' : '#e6e9f2'),
            border: '1px solid ' + (active ? '#6ea8fe' : '#2b3348'), borderRadius: '8px', opacity: disabled ? .55 : 1
          });
  ...
  UIK.register('nav', {
    ...
    mount: function (stage, ctx) {
      ...
      var track = U.el('div'); U.css(track, { position: 'absolute', left: '6%', right: '6%', top: '11px', height: '2px', background: '#2b3348' });
      var fill = U.el('div'); U.css(fill, { position: 'absolute', left: '0', top: '0', height: '100%', width: '0', background: '#6ea8fe', transition: UIK.isReduced() ? 'none' : 'width .35s cubic-bezier(.4,0,.2,1)' });
      track.appendChild(fill); line.appendChild(track);
      var steps = ['提交申请', '部门审批', '财务复核', '完成'].map(function (t, i) {
        var s = U.el('div'); U.css(s, { position: 'relative', textAlign: 'center', flex: '1', fontSize: '11px', color: '#6f7994', zIndex: 1 });
        var c = U.el('div'); U.css(c, {
          width: '24px', height: '24px', borderRadius: '50%', margin: '0 auto 6px', background: '#1c2130',
      ...
      function sync() {
        steps.forEach(function (s, i) {
          ...
          s.c.style.background = done ? '#4ade80' : (active ? '#6ea8fe' : '#1c2130');
          s.c.style.borderColor = done ? '#4ade80' : (active ? '#6ea8fe' : '#3a4255');
          s.c.style.color = (done || active) ? '#0b0d12' : '#98a1b8';
  ...
  UIK.register('nav', {
    ...
    mount: function (stage, ctx) {
      ...
      var sb = U.el('nav'); U.css(sb, {
        width: '148px', flex: '0 0 auto', background: '#161a24', border: '1px solid #2b3348', borderRadius: '12px',
      ...
      var items = [['◎', '工作台'], ['▤', '客户管理'], ['◈', '商机管理'], ['▣', '项目管理'], ['▦', '知识库'], ['⚙', '系统设置']].map(function (p, i) {
        ...
        U.css(a, {
          ...
          color: '#98a1b8', fontSize: '12.5px', cursor: 'pointer', borderRadius: '9px', whiteSpace: 'nowrap', textAlign: 'left'
        });
      ...
      var main = U.el('div'); U.css(main, {
        flex: '1', background: '#10141d', border: '1px solid #232b3a', borderRadius: '12px', display: 'flex',
        alignItems: 'center', justifyContent: 'center', color: '#98a1b8', fontSize: '12px'
      });
      ...
      function sync() {
        items.forEach(function (it, i) {
          var on = i === cur;
          it.a.style.background = on ? '#232b3d' : 'transparent';
          it.a.style.color = on ? '#6ea8fe' : '#98a1b8';
  ...
  UIK.register('nav', {
    ...
    mount: function (stage, ctx) {
      var phone = U.el('div'); U.css(phone, {
        width: '196px', height: '188px', borderRadius: '18px', border: '1px solid #2b3348', background: '#0f131c',
        display: 'flex', flexDirection: 'column', overflow: 'hidden'
      });
      var body = U.el('div'); U.css(body, {
        flex: '1', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#98a1b8', fontSize: '12px'
      });
      body.textContent = '首页';
      var bar = U.el('nav'); U.css(bar, {
        display: 'flex', borderTop: '1px solid #232b3a', background: '#141926', padding: '6px 0 8px'
      });
      var items = [['◎', '首页', 0], ['▤', '客户', 0], ['◈', '商机', 3], ['☺', '我的', 0]].map(function (p, i) {
        ...
        U.css(b, {
          ...
          gap: '3px', color: '#6f7994', fontSize: '10.5px', cursor: 'pointer', position: 'relative', padding: '4px 0', minHeight: '44px'
        });
        ...
        if (p[2]) {
          var badge = U.el('span'); U.css(badge, {
            ...
            borderRadius: '8px', background: '#f87171', color: '#fff', fontSize: '9.5px', lineHeight: '15px', textAlign: 'center', padding: '0 4px'
          });
      ...
      items.forEach(function (it, i) {
        ctx.on(it.b, 'click', function () {
          items.forEach(function (o, k) {
            o.b.style.color = k === i ? '#6ea8fe' : '#6f7994';
